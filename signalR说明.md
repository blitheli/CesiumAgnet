...
// 可选：加入会话后，服务端会对当前调用者发送 SessionJoined
...
| 方法名           | 作用                                                  | 调用示例                                                 |
| ------------- | --------------------------------------------------- | ---------------------------------------------------- |
| `JoinSession` | 将当前连接加入名为 `sessionId` 的组，之后才能收到该组的 `ReceiveCommand` | `await connection.invoke("JoinSession", sessionId);` |
| `SendCommand` | 向指定 `sessionId` 组广播一条指令（对象形状见下文 `CesiumCommand`）    | `await connection.invoke("SendCommand", command);`   |


**推荐顺序**：先 `start()`，再 `JoinSession(sessionId)`，再按需 `SendCommand(...)`。
...
## 6. 创建地面站：`createGroundStation` JSON 示例

以下为可直接用于 `invoke("SendCommand", ...)` 的示例（`sessionId` 请改为你的实际会话 ID）。
