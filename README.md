...
1. AI 驱动：通过自然语言指令（Agent）自动调用 Cesium API 创建实体。
  1. 即时响应：用户在网页窗口输入，视图立即更新。
  2. 二次开发：支持用户通过 Python 脚本远程驱动网页视图，实现自动化操控。
...
- `target`：指令作用对象，默认 `entity`（Cesium 实体层）；可预留 `vgt` 等其它目标。省略 `target` 时与显式写 `"entity"` 等价。
...
connection.on("ReceiveCommand", (data) => {
    const { target = "entity", action, params } = data;
    if (target !== "entity") return; // 其它 target 由对应适配器处理
